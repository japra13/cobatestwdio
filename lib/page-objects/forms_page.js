...
class FormsPage {
  ...
  async contentExist() {
    let contentExst = await $(keyForms.FORMS_CONTENT_POPUP);
    await contentExst.isExisting();
  }

  async popExist() {
    let alertMessage = await $(keyForms.FORMS_ALERT_TITLE_POPUP).getText();
    let messageElmnt = await $(keyForms.FORMS_MESSAGE_POPUP).getText();

    expect(alertMessage).toHaveText("This button is");
    expect(messageElmnt).toHaveText("This button is active");
  }

  async askMeLaterPopup() {
    let askMeLaterBtn = await $(keyForms.FORMS_CONTENT_ASKMELATER_BUTTON);
    await askMeLaterBtn.click();
  }

  async cancelPopup() {
    let cancelBtn = await $(keyForms.FORMS_CONTENT_CANCEL_BUTTON);
    await cancelBtn.click();
  }

  async okPopup() {
    let okBtn = await $(keyForms.FORMS_CONTENT_OK_BUTTON);
    await okBtn.click();
  }
}
