...
When(/^09_User memilih pilihan ASK ME LATER$/, async () => {
  await FormsPages.askMeLaterPopup();
});
...
When(/^10_User menekan kembali tombol buttons Active$/, async () => {
  await FormsPages.actvBtn();
});
...
When(/^11_User memilih pilihan CANCEL$/, async () => {
  await FormsPages.cancelPopup();
});
...
When(/^12_User menekan kembali buttons Active$/, async () => {
  await FormsPages.actvBtn();
});
...
When(/^13_User memilih pilihan OK$/, async () => {
  await FormsPages.okPopup();
});
